...
class SpeakerIntegrator:
    ...
    def __init__(
        ...
        Parameters
        ----------
        segment_duration_threshold : float, optional
            統合する話者区間の最小時間(秒), by default 60.0

        split_segment_duration : float, optional
            統合する話者区間の最小時間(秒), by default 1.0

        max_segment_duration : float, optional
            統合する話者区間の最大時間(秒), by default 120.0
    ...
    def integrate(
        self: "SpeakerIntegrator", segments: list[SpeakerSegment]
    ) -> list[SpeakerSegment]:
        """連続する話者区間を統合する."""
        if len(segments) < 1:
            return []
